...
**LOOP COMPLETE 2026-07-31 — all 20 tasks done. 30 production defects found+filed. make ci green 4384/0. Branch worktree-test-refactor ready for merge review.**
...
- **From task 16 (Phase 4 closeout)**: tests/ now = e2e (contracts+saga+
  reliability), testkit, fixtures, live, ci, lexicon_validation. T1 lives
  colocated (25 TestMains, one per package, floors verified). Stack
  control channel: .ci-out/stack-control-$PROJECT, watcher pidfile,
  5 argless verbs; requireSingleFeedTopology refuses poisoned stacks
  (reconcile = --force-recreate appview). packageParallelism STAYS 1 —
  the trigger comment was corrected: relocation didn't remove the
  account-event storm, the wantedCollections bypass did not change.
  DOCKER FLAKE CLASS: identical failure across runs landing BEFORE any
  test output (SIGKILL 137 / "missing dependency netns") = containerd
  state; full down -v + prune clears it. NEVER pgrep-wait on a command
  you launched (matches itself — two runs silently never started);
  sentinel files. OPEN ITEMS: hostedBy-verification defect ownership
  (phase 5 vs follow-up); tests/live's 34 network-tolerant skips need
  the task-20 §3.2 decision (explicit T3 + missing config should FAIL);
...
- **From tasks 16-17 (ops + patterns)**: NEVER resume a presumed-dead
  ...
  moderator/sortition.
- **From task 18**: VERIFY FILING CLAIMS — first report-vs-reality gap of
  the loop ("filed" meant "pinned"); reviewers + parent now spot-check the
  issues directory on any filing claim. Behavioral middleware classifier
  pattern (registration_test.go) = the routes gold standard; its rules:
  per-probe unique identity (shared counters race), probe-ceiling guard,
  declared⊆registered AND registered⊆declared. cmd/server's getComments is
  r.Handle (all verbs) — outside the table's jurisdiction, documented.
  Mutation-sweep rule: a "survivor" is a bad-replace-pattern suspect FIRST
  (str.replace hit a doc comment). Fixture rules: insert order must FIGHT
  expected order; ties straddle page boundaries; timeframe fixtures need a
  LADDER (one row per adjacent-window gap) or boundary mutations survive.
  Masked defects disclose reachability in pin + issue (5 of the 17 are
  masked). Comment cursors are unsigned where post cursors are HMAC'd —
  noted in the NaN pin. UpsertVote/AtomicVote opposite lock order = latent
  deadlock, filed. Defect tally 29; backlog dir at 42 files.
- **From task 19 (federation + fresh-fixer debut)**: the fresh-fixer
  protocol worked first try — its mutation-check found the OLD eventsProcessed
  bound would have PASSED the trust-config mutation (IndexUser dies on
  handle.invalid so no user row appears either), the NEW same-repo malformed-
  block dead-letter bound catches it. Rule cemented: negatives on a live
  stream must bound on a same-repo-same-subscription observable event, never
  on a counter that unfiltered identity/account events also advance.
  test-e2e-dev now -skips ^TestReliability|Federat (10 tests); RequireFederatedPDS
  is soft (nil when PDS2_URL unset) so the dev hatch stays coherent, hard
  (NewFederatedPDS fatals by name) when a federation contract is actually
  reached. compose logs / stack_capture_logs need COVES_CI_GOARCH (via
  stack_detect_arch) before hand-invocation. pds.log/pds2.log empty in CI
  (LOG_ENABLED=false) — the blocker if PDS diagnostics ever wanted.
  TASK-20 CARRY: spec §3.4a "(Phase 5 scope)" parenthetical + §3.7 endpoint
  list want a refresh now federation landed; audit residue to burn to zero
  = sleeps (oauth_integration_test.go:843, community_suggestion_integration_test.go:454
  + others), t.Skip (34 in tests/live + facet_test.go unreachable) — tests/live's
  network-tolerant skips need the §3.2 decision (explicit T3 + missing config
  should FAIL not skip). -allow-pending flip call site: check_contract_manifest
  in runner-ready.sh (task 20 VERIFIES it stayed flipped). Audit dialer floor
  = 1 (production connector.go — exempt). Defect tally 30; backlog 42 files.