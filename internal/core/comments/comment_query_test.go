...
func TestCommentQuery_BasicFetch(t *testing.T) {
	...
	testUser := fixtures.User(t, db, "basicfetch.test", "did:plc:basicfetch123")
	testCommunity, err := fixtures.Community(ctx, db, "basicfetchcomm", "ownerbasic.test")
	require.NoError(t, err, "Failed to create test community")

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Basic Fetch Test Post", 0, time.Now())
...
func TestCommentQuery_NestedReplies(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "nested.test", "did:plc:nested123")
	testCommunity, err := fixtures.Community(ctx, db, "nestedcomm", "ownernested.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Nested Test Post", 0, time.Now())
...
func TestCommentQuery_DepthLimit(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "depth.test", "did:plc:depth123")
	testCommunity, err := fixtures.Community(ctx, db, "depthcomm", "ownerdepth.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Depth Test Post", 0, time.Now())
...
func TestCommentQuery_HotSorting(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "hot.test", "did:plc:hot123")
	testCommunity, err := fixtures.Community(ctx, db, "hotcomm", "ownerhot.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Hot Sorting Test", 0, time.Now())
...
func TestCommentQuery_TopSorting(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "top.test", "did:plc:top123")
	testCommunity, err := fixtures.Community(ctx, db, "topcomm", "ownertop.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Top Sorting Test", 0, time.Now())
...
func TestCommentQuery_NewSorting(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "new.test", "did:plc:new123")
	testCommunity, err := fixtures.Community(ctx, db, "newcomm", "ownernew.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "New Sorting Test", 0, time.Now())
...
func TestCommentQuery_Pagination(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "page.test", "did:plc:page123")
	testCommunity, err := fixtures.Community(ctx, db, "pagecomm", "ownerpage.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Pagination Test", 0, time.Now())
...
func TestCommentQuery_EmptyThread(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "empty.test", "did:plc:empty123")
	testCommunity, err := fixtures.Community(ctx, db, "emptycomm", "ownerempty.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Empty Thread Test", 0, time.Now())
...
func TestCommentQuery_DeletedComments(t *testing.T) {
	...
	testUser := fixtures.User(t, db, "deleted.test", "did:plc:deleted123")
	testCommunity, err := fixtures.Community(ctx, db, "deletedcomm", "ownerdeleted.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Deleted Comments Test", 0, time.Now())
...
func TestCommentQuery_InvalidInputs(t *testing.T) {
	...
	testUser := fixtures.User(t, db, "validation.test", "did:plc:validation123")
	testCommunity, err := fixtures.Community(ctx, db, "validationcomm", "ownervalidation.test")
	require.NoError(t, err, "Failed to create test community")
	validPostURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Validation Test Post", 0, time.Now())
...
func TestCommentQuery_HTTPHandler(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "http.test", "did:plc:http123")
	testCommunity, err := fixtures.Community(ctx, db, "httpcomm", "ownerhttp.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "HTTP Handler Test", 0, time.Now())
...
func TestCommentQuery_ParentRkeySubtree(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "subtree.test", "did:plc:subtree123")
	testCommunity, err := fixtures.Community(ctx, db, "subtreecomm", "ownersubtree.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Subtree Test Post", 0, time.Now())
	...
	t.Run("ParentNotFound for rkey of comment in another post", func(t *testing.T) {
		otherPostURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Other Post", 0, time.Now())
	...
	t.Run("Pagination of parent's direct children", func(t *testing.T) {
		pagPostURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Subtree Pagination Post", 0, time.Now())
	...
	t.Run("Rkey collision resolves to earliest indexed", func(t *testing.T) {
		collisionPostURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Collision Post", 0, time.Now())
		otherUser := fixtures.User(t, db, "subtree2.test", "did:plc:subtree456")

		// Two commenters use the same rkey within the same post (astronomically unlikely with TIDs)
		sharedRkey := testkit.TID()
	...
	t.Run("Deleted parent renders as placeholder with children intact", func(t *testing.T) {
		...
		delPostURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Deleted Parent Post", 0, time.Now())
...
func TestCommentQuery_ParentRkeyHTTPHandler(t *testing.T) {
	...
	ctx := context.Background()
	testUser := fixtures.User(t, db, "subtreehttp.test", "did:plc:subtreehttp123")
	testCommunity, err := fixtures.Community(ctx, db, "subtreehttpcomm", "ownersubtreehttp.test")
	require.NoError(t, err)

	postURI := fixtures.Post(t, db, testCommunity, testUser.DID, "Subtree HTTP Test", 0, time.Now())
