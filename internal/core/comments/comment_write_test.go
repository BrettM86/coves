...
func TestCommentWrite_CreateTopLevelComment(t *testing.T) {
	...
	author := testkit.NewPDS(t).CreateAccount(t, testkit.WithHandlePrefix("cmw"))
	testUserHandle, userDID, pdsAccessToken := author.Handle, author.DID, author.AccessToken

	// Index user in AppView
	testUser := fixtures.User(t, db, testUserHandle, userDID)

	// Create test community and post to comment on
	testCommunityDID, err := fixtures.Community(ctx, db, "test-community", "owner.test")
	if err != nil {
		t.Fatalf("Failed to create test community: %v", err)
	}

	postURI := fixtures.Post(t, db, testCommunityDID, testUser.DID, "Test Post", 0, time.Now())
	postCID := "bafypost123"

	// Create mock OAuth session for service layer
	mockStore := fixtures.NewOAuthStore()
...
func TestCommentWrite_CreateNestedReply(t *testing.T) {
	...
	// Create test user
	replier := testkit.NewPDS(t).CreateAccount(t, testkit.WithHandlePrefix("rpl"))
	testUserHandle, userDID, pdsAccessToken := replier.Handle, replier.DID, replier.AccessToken

	testUser := fixtures.User(t, db, testUserHandle, userDID)

	// Create test post and parent comment
	testCommunityDID, err := fixtures.Community(ctx, db, "reply-community", "owner.test")
	require.NoError(t, err, "creating the community the reply hangs off")
	postURI := fixtures.Post(t, db, testCommunityDID, testUser.DID, "Test Post", 0, time.Now())
	postCID := "bafypost456"
...
func TestCommentWrite_UpdateComment(t *testing.T) {
	...
	// Create test user
	updater := testkit.NewPDS(t).CreateAccount(t, testkit.WithHandlePrefix("upd"))
	userDID, pdsAccessToken := updater.DID, updater.AccessToken
...
func TestCommentWrite_DeleteComment(t *testing.T) {
	...
	// Create test user
	deleter := testkit.NewPDS(t).CreateAccount(t, testkit.WithHandlePrefix("del"))
	userDID, pdsAccessToken := deleter.DID, deleter.AccessToken
...
func TestCommentWrite_CannotUpdateOthersComment(t *testing.T) {
	...
	owner := pdsServer.CreateAccount(t, testkit.WithHandlePrefix("own"))
	ownerDID := owner.DID

	// Create second user (attacker)
	attacker := pdsServer.CreateAccount(t, testkit.WithHandlePrefix("atk"))
	attackerDID, attackerToken := attacker.DID, attacker.AccessToken
...
func TestCommentWrite_ConcurrentModificationDetection(t *testing.T) {
	...
	// Create test user
	concurrency := testkit.NewPDS(t).CreateAccount(t, testkit.WithHandlePrefix("cnc"))
	userDID, pdsAccessToken := concurrency.DID, concurrency.AccessToken
