...
func TestBlobUpload_E2E_PostWithImages(t *testing.T) {
	t.Parallel()
	// No PDS health check and no skip: TestMain's RequirePDS already probed it
	// and failed the whole binary if it was absent. §3.1 — asking for the
	// integration tier IS asking for the infrastructure, so a missing PDS is a
	// failure, and the skip this replaced is exactly how the collision below
	// stayed hidden.
...
func TestBlobUpload_E2E_CommentWithImage(t *testing.T) {
	...
	postURI := fixtures.Post(t, db, community.DID, author.DID, "Post for Comment Test", 0, time.Now())
...
func createTestCommunityWithBlobCredentials(t *testing.T, repo communities.Repository, suffix string) *communities.Community {
	...
	// A REAL PDS account for the community rather than fake credentials: the
	// blob path signs its uploads with them, so an invented token proves
	// nothing.
	//
	// testkit owns the handle, and that is load-bearing rather than tidiness.
	// Three tests in this file call this helper; the hand-rolled version it
	// replaced built the local label from time.Now().Unix(), so at second
	// granularity they collided within one run ("Handle already taken") — which
	// is how blob upload for comments and blob validation came to be silently
	// skipped. testkit.CreateAccount generates from the run prefix plus an
	// atomic counter, inside the PDS' 18-character local-label cap, and takes
	// the handle domain from the stack instead of hardcoding one (§3.7).
	//
	// A failure here is fatal, not a skip: TestMain already established the PDS
	// floor, so this can only be a defect.
	account := testkit.NewPDS(t).CreateAccount(t, testkit.WithHandlePrefix("blob"))
	communityDID, accessToken := account.DID, account.AccessToken
	communityName, _, _ := strings.Cut(account.Handle, ".")

	t.Logf("✓ Created real PDS account: handle=%s DID=%s", account.Handle, communityDID)
