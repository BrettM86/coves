...
func TestUserProfileAvatarE2E_UpdateWithAvatar(t *testing.T) {
	...
	eventLoop:
		...
		if realEvent.Kind == "commit" && realEvent.Commit != nil {
			...
			_, updateErr := userService.UpdateProfile(ctx, userDID, displayNamePtr, bioPtr, avatarCIDPtr, bannerCIDPtr)
			if updateErr != nil {
...
func TestUserProfileAvatarE2E_UpdateWithBanner(t *testing.T) {
	...
	eventLoop:
		...
		if realEvent.Kind == "commit" && realEvent.Commit != nil {
			...
			_, _ = userService.UpdateProfile(ctx, userDID, displayNamePtr, bioPtr, avatarCIDPtr, bannerCIDPtr)
		}
...
func TestUserProfileAvatarE2E_UpdateDisplayNameAndBio(t *testing.T) {
	...
	eventLoop:
		...
		if realEvent.Kind == "commit" && realEvent.Commit != nil {
			...
			_, _ = userService.UpdateProfile(ctx, userDID, displayNamePtr, bioPtr, nil, nil)
		}
...
func TestUserProfileAvatarE2E_ReplaceAvatar(t *testing.T) {
	...
	t.Run("replace existing avatar with new one", func(t *testing.T) {
		...
		_, _ = userService.UpdateProfile(ctx, userDID, &displayName, nil, &initialAvatarCID, nil)
		...
		_, _ = userService.UpdateProfile(ctx, userDID, nil, nil, &newAvatarCID, nil)
