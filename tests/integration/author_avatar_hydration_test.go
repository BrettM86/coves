...
func TestAuthorProfileHydration(t *testing.T) {
	...
	communityDID, err := createFeedTestCommunity(db, ctx, fmt.Sprintf("avhydr-%s", testID), fmt.Sprintf("owner-%s.test", testID))
	require.NoError(t, err)

	authorDID := fmt.Sprintf("did:plc:avhydr%s", testID)
	postURI := createTestPost(t, db, communityDID, authorDID, "Author hydration post", 1, time.Now().Add(-1*time.Hour))
	...
	t.Run("AuthorWithoutProfileStaysBare", func(t *testing.T) {
		...
		bareURI := createTestPost(t, db, communityDID, bareDID, "Bare author post", 1, time.Now())
